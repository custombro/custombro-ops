...
$RunRoot = Join-Path $LabRoot ('financial_reconciliation_v4_' + $Stamp)
$Report = Join-Path $RunRoot 'FINANCIAL_RECONCILIATION_V4.txt'
$Contract = Join-Path $RunRoot 'FINANCIAL_CONTRACT_V4.json'
$Helper = Join-Path $RunRoot 'financial_reconciler_v4.py'
$HelperUrl = 'https://raw.githubusercontent.com/custombro/custombro-ops/main/scripts/helpers/autotrade_v2_2_financial_reconciler_v4.py'
...
'@
    $Temporary = Join-Path $env:TEMP (
        'autotrade_financial_reconciliation_order_count_' +
...
Add-Result 'EXECUTION_MODE' 'V2_2_FINANCIAL_RECONCILIATION_V4_READ_ONLY'
Add-Result 'REGEX_GROUP_CONFLICT_DETECTION_REPLACED' $true
Add-Result 'EXACT_TRADE_ROW_FIELD_RECONCILIATION' $true
...
try {
    ...
    Add-Result 'FINANCIAL_RECONCILER_COMPILE_EXIT_CODE' $CompileExit

    if ($CompileExit -ne 0) {
        throw ('FINANCIAL_RECONCILER_COMPILE_FAILED:' + ($CompileOutput -join ' '))
    }
    Add-Result 'FINANCIAL_RECONCILER_COMPILE' 'PASS'

    $ReconcileOutput = @(
        & $Python $Helper $Source $Contract 2>&1
    )
    $ReconcileExit = $LASTEXITCODE
    $ReconcileText = $ReconcileOutput -join [Environment]::NewLine
    Add-Result 'FINANCIAL_RECONCILER_EXIT_CODE' $ReconcileExit

    if ($ReconcileExit -ne 0) {
        throw ('FINANCIAL_RECONCILER_FAILED:' + $ReconcileText)
    }
    if (-not $ReconcileText.Contains('FINAL_STATUS=PASS_FINANCIAL_RECONCILIATION_V4')) {
        throw ('FINANCIAL_RECONCILER_PASS_MARKER_MISSING:' + $ReconcileText)
    }

    foreach ($Line in $ReconcileOutput) {
        $Text = [string]$Line
        if (
            $Text.StartsWith('OFFICIAL_PNL_SOURCE=') -or
            $Text.StartsWith('TRADE_ROWS_PATH=') -or
            $Text.StartsWith('RAW_TRADE_ROW_COUNT=') -or
            $Text.StartsWith('CANONICAL_TRADE_ROW_COUNT=') -or
            $Text.StartsWith('UNIQUE_TRADE_ROW_COUNT=') -or
            $Text.StartsWith('DUPLICATE_TRADE_ROW_COUNT=') -or
            $Text.StartsWith('LATEST_BUSINESS_DATE=') -or
            $Text.StartsWith('LATEST_DATE_TRADE_ROW_COUNT=') -or
            $Text.StartsWith('SAME_ROW_CONFLICT_COUNT=') -or
            $Text.StartsWith('MISSING_COMPONENT_ROW_COUNT=') -or
            $Text.StartsWith('EXACT_COMPONENTS_READY=') -or
            $Text.StartsWith('DAILY_NET_READY=') -or
            $Text.StartsWith('OFFICIAL_DAILY_NET_VALUE=') -or
            $Text.StartsWith('LATEST_DATE_ROW_NET_SUM=') -or
            $Text.StartsWith('DAILY_TO_ROW_NET_DIFFERENCE=') -or
            $Text.StartsWith('INTRADAY_GRAPH_READY=') -or
            $Text.StartsWith('DAILY_WEEK_MONTH_GRAPH_READY=') -or
            $Text.StartsWith('FINANCIAL_SEMANTIC_READY=') -or
            $Text.StartsWith('FINANCIAL_CONTRACT=')
        ) {
    ...
    if ($SourceHashAfter -ne $SourceHashBefore) {
        throw 'OFFICIAL_PNL_SOURCE_CHANGED_DURING_AUDIT'
    }
    ...
    Add-Result 'RUNTIME_RESTARTED' $false
    Add-Result 'V2_2_LIVE_INSTALL_ALLOWED' $false
    Add-Result 'NEXT_STAGE' 'BUILD_V2_2_OFFLINE_PREVIEW_FROM_V4_FINANCIAL_AND_V3_STRATEGY_CANDIDATE_CONTRACTS'
    Add-Result 'FINAL_STATUS' 'PASS_V2_2_FINANCIAL_RECONCILIATION_V4_NO_LIVE_MUTATION'
    $Success = $true
}
catch {
    Add-Result 'FINANCIAL_RECONCILIATION_ERROR' $_.Exception.Message
    ...
    Add-Result 'RUNTIME_RESTARTED' $false
    Add-Result 'V2_2_LIVE_INSTALL_ALLOWED' $false
    Add-Result 'FINAL_STATUS' 'FAIL_V2_2_FINANCIAL_RECONCILIATION_V4_NO_LIVE_MUTATION'
}
