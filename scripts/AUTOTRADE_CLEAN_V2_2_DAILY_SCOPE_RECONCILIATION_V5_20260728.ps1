...
$RunRoot = Join-Path $LabRoot ('daily_scope_v5_' + $Stamp)
$Report = Join-Path $RunRoot 'DAILY_SCOPE_RECONCILIATION_V5.txt'
$Contract = Join-Path $RunRoot 'DAILY_SCOPE_CONTRACT_V5.json'
$Helper = Join-Path $RunRoot 'daily_scope_reconciler_v5.py'
$HelperUrl = 'https://raw.githubusercontent.com/custombro/custombro-ops/main/scripts/helpers/autotrade_v2_2_daily_scope_reconciler_v5.py'
...
'@
    $Temporary = Join-Path $env:TEMP (
        'autotrade_daily_scope_order_count_' +
...
Add-Result 'EXECUTION_MODE' 'V2_2_DAILY_SCOPE_RECONCILIATION_V5_READ_ONLY'
Add-Result 'V4_DAILY_SUMMARY_ACCEPTANCE_REVOKED' $true
Add-Result 'CANONICAL_DAILY_SOURCE_POLICY' 'TRADE_ROWS_GROUPED_BY_BUSINESS_DATE'
...
try {
    ...
    Add-Result 'DAILY_SCOPE_RECONCILER_COMPILE_EXIT_CODE' $CompileExit
    if ($CompileExit -ne 0) {
        throw ('DAILY_SCOPE_RECONCILER_COMPILE_FAILED:' + ($CompileOutput -join ' '))
    }
    Add-Result 'DAILY_SCOPE_RECONCILER_COMPILE' 'PASS'

    $ReconcileOutput = @(& $Python $Helper $Source $Contract 2>&1)
    $ReconcileExit = $LASTEXITCODE
    $ReconcileText = $ReconcileOutput -join [Environment]::NewLine
    Add-Result 'DAILY_SCOPE_RECONCILER_EXIT_CODE' $ReconcileExit

    if ($ReconcileExit -ne 0) {
        throw ('DAILY_SCOPE_RECONCILER_FAILED:' + $ReconcileText)
    }
    if (-not $ReconcileText.Contains('FINAL_STATUS=PASS_DAILY_SCOPE_RECONCILIATION_V5')) {
        throw ('DAILY_SCOPE_RECONCILER_PASS_MARKER_MISSING:' + $ReconcileText)
    }
    ...
    foreach ($Line in $ReconcileOutput) {
        ...
        if (
            ...
            $Text.StartsWith('LATEST_') -or
            $Text.StartsWith('ACCOUNTING_') -or
            $Text.StartsWith('PERIOD_OR_UNKNOWN_') -or
            $Text.StartsWith('SUMMARY_') -or
            $Text.StartsWith('CANONICAL_LATEST_DAY_SOURCE=') -or
            $Text.StartsWith('DAILY_') -or
            $Text.StartsWith('INTRADAY_') -or
            $Text.StartsWith('FINANCIAL_DAILY_CONTRACT_READY=')
        ) {
    ...
    Add-Result 'RUNTIME_RESTARTED' $false
    Add-Result 'V2_2_LIVE_INSTALL_ALLOWED' $false
    Add-Result 'NEXT_STAGE' 'READ_ONLY_INTRADAY_FILL_LINKAGE_AUDIT_THEN_OFFLINE_PREVIEW'
    Add-Result 'DAILY_SCOPE_CONTRACT' $Contract
    Add-Result 'FINAL_STATUS' 'PASS_V2_2_DAILY_SCOPE_RECONCILIATION_V5_NO_LIVE_MUTATION'
    $Success = $true
}
catch {
    Add-Result 'DAILY_SCOPE_ERROR' $_.Exception.Message
    ...
    Add-Result 'RUNTIME_RESTARTED' $false
    Add-Result 'V2_2_LIVE_INSTALL_ALLOWED' $false
    Add-Result 'FINAL_STATUS' 'FAIL_V2_2_DAILY_SCOPE_RECONCILIATION_V5_NO_LIVE_MUTATION'
}
